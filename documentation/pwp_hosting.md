...
- ssh into the server
    - For Mac and Linux: `ssh root@srilathaa.com`
    - For Windows: Set up a new PuTTY session
        ...
        - **Session**
            ...
            - Click Save
- create user
    - `useradd -m -s /bin/bash -g users -G sudo salla`
...
- change ownership of the .ssh directory
    - `cd /home/salla`
    - `chown -R salla:users .ssh/`


- Test Changes
    - `exit`
    - Mac/Linux: `ssh salla@srilathaa.com`
    - Windows:
        - **Session**
            - Click pwp
            - Click Load
...
- create a directory for your deployment
    - `sudo mkdir -p /var/www/srilathaa.com/html`
- fix permissions
    - `sudo chown -R $USER:users /var/www/srilathaa.com/html`
...
- `sudo vim /etc/nginx/sites-available/srilathaa.com`
...
server {
    ...
    root /var/www/srilathaa.com/html;
    index index.html index.htm index.nginx-debian.html;

    server_name srilathaa.com www.srilathaa.com;
...
- create a symlink to the sites-enabled directory
    - `sudo ln -s /etc/nginx/sites-available/srilathaa.com /etc/nginx/sites-enabled/`
...
- Create a new ssh key on your server in the .ssh directory
    - `ssh-keygen -t ed25519 -C  "github Deploy Key" -f /home/salla/.ssh/github_deploy_key`
    - Don't enter a passphrase
        - hit enter twice
- Copy the public key into your authorized keys file
    - `cat /home/salla/.ssh/github_deploy_key.pub >> /home/salla/.ssh/authorized_keys`
...
- Copy the private key to your clipboard **(this is a very dangerous operation do not share this key with anyone )**
    ...
    - `cat /home/salla/.ssh/github_deploy_key`
  ...
  deploy:
    ...
    steps:
      ...
      - name: Deploy to server
        run: |
         ssh-keyscan -H srilathaa.com >> ~/.ssh/known_hosts 
         scp -r ./dist/* gkephart@srilathaa.com:/var/www/srilathaa.com/html
```
